...
export default function ProjectGuide(): ReactElement {
  ...
  return (
    <div className="container pgd-page">
        ...
        <aside className="pgd-sidebar">
          ...
          <nav className="pgd-sidebar__nav">
            {PROJECT_DATA.map((p) => (
              ...
              >
                <span className="pgd-sidebar__icon" style={{ background: `${p.color}1a`, color: p.color }}>{p.icon}</span>
                <span className="pgd-sidebar__title">{p.title}</span>
              </button>
            ))}
          </nav>
        </aside>
          ...
          <div className="pgd-hero-card" style={{ borderLeftColor: project.color }}>
            ...
            <div>
              ...
              <div className="pgd-hero-tags">
                {project.solarApi.endpoints.map((ep, i) => (
                  <span key={i} className="pgd-tech-badge">{ep.name.split(' (')[0]}</span>
                ))}
              </div>
            </div>
          </div>
